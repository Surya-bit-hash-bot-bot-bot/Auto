...
async def start(client, message: Message):
    ...
    # If verification is required, check user's status
    if not await check_verification(client, message.from_user.id) and VERIFY:
        verification_button = InlineKeyboardMarkup([
            [
                InlineKeyboardButton("Verify", url=await get_token(client, message.from_user.id, f"https://telegram.me/{BOT_USERNAME}?start=verify")),
    ...
    # Normal flow continues if user is verified
...
@Client.on_message(filters.private & filters.command("start"))
async def handle_verification(client, message: Message):
